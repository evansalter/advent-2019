...
func (p point) Steps(targetPoint point, wirePoints [][]point) int {
	count := 0
	for _, w := range wirePoints {
		for _, p := range w {
			count++
			if p == targetPoint {
				break
...
func (p point) ToPointSteps(targetPoint point, wirePoints [][]point) pointDistance {
	return pointDistance{
		P:    p,
		Dist: p.Steps(targetPoint, wirePoints),
	}
}
...
func Run() {
	...
	for p, c := range pointCountMap {
		if allIntsInSliceNonZero(c) {
			pointDistances = append(pointDistances, p.ToPointSteps(p, wirePoints))
		}
	}
